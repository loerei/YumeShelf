...
function getOrCreateTranslationCard() {
...
function renderTranslationQueue(card: HTMLElement, payload: any, electronAPI: ElectronAPI) {
...
function handleCancelButton(cancelBtn: HTMLButtonElement, payload: any, electronAPI: ElectronAPI) {
...
function setProgressCardStatus(
    titleEl: HTMLElement,
    ...
    const jobName = payload.activeJobName;
    ...
    switch (payload.status) {
        ...
        case 'sync-extracting':
            titleEl.textContent = jobName ? `Extracting: ${jobName}` : 'Syncing Translation';
            messageEl.textContent = 'Scanning database and maps for strings...';
            fillEl.style.width = '0%';
            break;
            
        case 'syncing':
            titleEl.textContent = jobName ? `Syncing: ${jobName}` : 'Syncing Translation';
        ...
        case 'synced':
            titleEl.textContent = jobName ? `Synced: ${jobName}!` : 'Translation Synced!';
...
function updateTranslationCardContent(card: HTMLElement, payload: any, electronAPI: ElectronAPI) {
    ...
    if (cancelBtn) {
        handleCancelButton(cancelBtn, payload, electronAPI);
    }

    if (titleEl && messageEl && fillEl) {
        setProgressCardStatus(titleEl, messageEl, fillEl, card, payload);
    }
}
...
export function bindIpcEvents({
    ...
    function handleBlockingTranslationStatus(payload: any) {
    ...
    electronAPI.on('translation-status', (payload: any) => {
        if (handleBlockingTranslationStatus(payload)) {
            return;
        }
        ...
        if (['sync-extracting', 'syncing', 'synced', 'sync-error', 'sync-cancelled', 'sync-queued'].includes(payload.status)) {
            const card = getOrCreateTranslationCard();
            if (card) {
                card.style.display = 'flex';
                renderTranslationQueue(card, payload, electronAPI);
                updateTranslationCardContent(card, payload, electronAPI);
            }
        }
    });
}
