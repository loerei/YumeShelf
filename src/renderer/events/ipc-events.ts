...
export function bindIpcEvents({
    ...
    function renderTranslationQueue(card: HTMLElement, payload: any) {
    ...
    function updateTranslationCardContent(card: HTMLElement, payload: any) {
    ...
    electronAPI.on('translation-status', (payload: any) => {
        ...
        if (['sync-extracting', 'syncing', 'synced', 'sync-error', 'sync-cancelled', 'sync-queued'].includes(payload.status)) {
            ...
            if (card) {
                ...
                renderTranslationQueue(card, payload);
                updateTranslationCardContent(card, payload);
            }
        }
    });
}
