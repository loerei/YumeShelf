...
- [title-resolver] **Notice on Custom Game Names**: With the transition to dynamic title resolution, legacy database records are now refreshed to reflect official metadata titles. If you manually renamed games in previous versions, these names may be reset to the newly resolved titles (we sincerely apologize for this one-time inconvenience!). Moving forward, all manual renames via the Rename action are explicitly flagged and permanently preserved.
